...
# Get Volume
get_volume() {
    volume=$(pactl get-sink-volume @DEFAULT_SINK@ | grep -Eo '[0-9]+%' | head -1 | sed 's/%//')
    if [[ "$volume" -eq "0" ]]; then
        echo "Muted"
    else
        echo "$volume%"
...
inc_volume() {
    if [ "$(pactl get-sink-mute @DEFAULT_SINK@)" == "Mute: yes" ]; then
        pactl set-sink-mute @DEFAULT_SINK@ 0 && notify_user
    fi
...
toggle_mic() {
	if [ "$(pactl get-source-mute @DEFAULT_SOURCE@)" == "Mute: no" ]; then
		...
		pactl set-sink-mute @DEFAULT_SINK@ 0 --default-source u && notify-send -e -u low -i "$iDIR/microphone.png" "Microphone Switched ON"
	fi
}
# Get Mic Icon
get_mic_icon() {
    current=$(pactl get-source-volume @DEFAULT_SOURCE@ | grep -Eo '[0-9]+%' | head -1 | sed 's/%//')
...
get_mic_volume() {
    volume=$(pactl get-source-volume @DEFAULT_SOURCE@ | grep -Eo '[0-9]+%' | head -1 | sed 's/%//')
    if [[ "$volume" -eq "0" ]]; then
        echo "Muted"
    else
        echo "$volume%"
...
inc_mic_volume() {
    if [ "$(pactl get-source-mute @DEFAULT_SOURCE@)" == "Mute: yes" ]; then
        pactl set-source-mute @DEFAULT_SOURCE@ 0 && notify_mic_user
    fi
